...
switch (_side) do
{
	case west:
	{
		...
		*/
		_query = format["SELECT playerid, name, life_cop_cash, life_cop_atmcash, coplevel, cop_licenses, cop_gear, adminlevel, blacklist, donatorlvl FROM players WHERE playerid='%1'",_uid];
	};
	...
	case civilian:
	{
		...
		*/
		_query = format["SELECT playerid, name, life_cash, life_atmcash, civ_licenses, arrested, adminlevel, donatorlvl, civ_gear FROM players WHERE playerid='%1'",_uid];
	};
};
