...
switch (_side) do
{
	case west:
	{
		...
		_query = format["INSERT INTO players (playerid, name, life_cop_cash, life_cop_atmcash, cop_gear, aliases, cop_licenses, civ_licenses, civ_gear) VALUES('%1', '%2', '%3', '%4', '%5', '%6', '""[]""', '""[]""', '""[]""')",_uid,_name,_money,_bank,_misc,_alias];
	};
	
	case civilian:
	{
		if(typeName _misc == "BOOL") then {_misc = [_misc] call DB_fnc_bool;};
		_query = format["INSERT INTO players (playerid, name, life_cash, life_atmcash, arrested, aliases, civ_gear, cop_licenses, civ_licenses, cop_gear) VALUES('%1', '%2', '%3', '%4', '%5', '%6','""[]""', '""[]""', '""[]""', '""[]""')",
		_uid, //1
		_name, //2 
		_money,//3
		_bank, //4
